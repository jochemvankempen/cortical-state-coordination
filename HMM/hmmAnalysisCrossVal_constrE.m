function hmmAnalysisCrossVal_constrE( mu, chanPerArea, binSize, timeEpoch, minNumState, maxNumState, numFold, recording, resultSave )
% hmmAnalysisCrossVal_constrE( mu, chanPerArea, binSize, timeEpoch, maxNumState, numFold, layerData, resultSave )
%
...
for numState = minNumState:maxNumState
    ...
    if 1%~exist(fileName,'file') || resultSave.overWriteExistingFiles
        fprintf('testing numstate %1d\n', numState)
        tStart = tic;
        fprintf('estimating parameters\n')
        [estTrans, estEmis, estPi0, aic(numState), bic(numState)] = fitHMM_constrE( emissionSeq, chanPerArea, numState, binSize, numIter );
        % determine states and state probabilities
        states = cell(numTrial,1);
        Pstates = cell(numTrial,1);
        fprintf('fitting model\n')
        parfor iTrial = 1:numTrial
        ...
        [ bootstat, bootTrans, bootEmis, bootPi0 ] = bootstrapLL(emissionSeq, chanPerArea, bootsam, numState, binSize, numIter);
...
for numState = minNumState:maxNumState
    ...
    if ~exist(fileName,'file') || resultSave.overWriteExistingFiles
        ...
        parfor iFold = 1:numFold
            ...
            [estTrans, estEmis, estPi0] = fitHMM_constrE( trainSeq, chanPerArea, numState, binSize, numIter );
...
function [ bootstat, bootTrans, bootEmis, bootPi0 ] = bootstrapLL(emissionSeq, chanPerArea, bootsam, numState, binSize, numIter)
...
parfor iBoot = 1:nBoot
    [bootTrans(iBoot,:,:), bootEmis(iBoot,:,:), bootPi0(iBoot,:), bootaic(iBoot,1), bootbic(iBoot,1)] = ...
        fitHMM_constrE( emissionSeq(bootsam(:,iBoot)), chanPerArea, numState, binSize, numIter );
end;
