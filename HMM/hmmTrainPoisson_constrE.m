function [guessTR,guessE,guessPi0,logliks] = hmmTrainPoisson_constrE(seqs,guessTR,guessE, guessPi0, constrE, varargin)
...
%   is emitted from state K. guessPi0 is the gues for the initial state
%   distribution
%
...
%   $Revision: 1.1.8.5 $  $Date: 2011/10/15 02:06:22 $
...
% check size of the matrix with constraints on the emission matrix
[checkEconstr, numEconstr] = size(constrE);
if checkEconstr ~= numStates
    error(message('stats:hmmtrain:InputSizeMismatch'));
end
if numEconstr ~= numEmissions
    error(message('stats:hmmtrain:InputSizeMismatch'));
end
% find number of emission constraints
constr = unique(constrE(:));

% enforce constraints in the initial emission matrix by assigning to averages
for iCon = 1:length(constr)
    for l = 1:numEmissions
        ind = constrE(:,l)==constr(iCon);
        temp = mean(guessE(ind,l));
        guessE(ind,l) = temp;
    end
end

% number of elements in guessPi0 should be equal to the number of states
checkPi0 = length(guessPi0);
...
if isnumeric(seqs)
    [numSeqs, N, seqLength] = size(seqs);
    cellflag = false;
elseif iscell(seqs)
    numSeqs = numel(seqs);
    allN = cellfun(@(x) size(x,1), seqs);
    if (all(allN==allN(1)))
        N = allN(1);
    else
        error('Number of neurons in the emission sequence should be the same on all trials');
    end;
    cellflag = true;
else
...
for iteration = 1:maxiter
    ...
    oldGuessE = guessE;
    oldGuessTR = guessTR;
    %oldGuessPi0 = guessPi0;
    normE = zeros( numStates, 1 );
    for count = 1:numSeqs
        if cellflag
            seq = seqs{count};
        ...
        if seqLength>1
            if baumwelch   % Baum-Welch training
                ...
                [~,logGE,logPseq,fs,bs,scale] =  hmmDecodePoisson(seq,guessTR,guessE,guessPi0);
                ...
                for k = 1:numStates
                    PI0(k) = exp( logf(k,1) + logb(k,1))./scale(2);
                end;

                for k = 1:numStates
                    for l = 1:numStates
                        for i = 1:seqLength-1
                            TR(k,l) = TR(k,l) + exp( logf(k,i) + logGTR(k,l) + logGE(l,i+1) + logb(l,i+1))./scale(i+1);
                        end
                    end
                end
                for k = 1:numStates
                    for l = 1:numEmissions
                        for i = 1:seqLength-1
                            E(k,l) = E(k,l) + seq(l,i+1)*exp( logf(k,i) +logb(k,i) );
                        end;
                    end
                    normE(k) = normE(k) + sum(exp( logf(k,:) +logb(k,:) ));
                end
    ...
    %guessE = E./(repmat(normE,1,numEmissions));
    % update constrained emission matrix
    for iCon = 1:length(constr)
        for l = 1:numEmissions
            ind = constrE(:,l)==constr(iCon);
            guessE(ind,l) = sum(E(ind,l)) / sum(normE(ind));
        end
    end
    guessTR  = TR./(repmat(totalTransitions,1,numStates));
    guessPi0 = PI0./sum(PI0);
